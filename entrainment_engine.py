    ...
    def validate_frequency(target_freq: float, refresh_rate: int, waveform: WaveformType) -> Tuple[float, WaveformType]:
        ...
        if waveform == WaveformType.SQUARE:
            ...
            if 10 <= target_freq <= 30:
                logger.warning(f"⚠ SAFETY WARNING: {target_freq} Hz Square Wave is in the high-risk photosensitive window (10-30 Hz).")
...
class EntrainmentEngine:
    ...
    def initialize_hardware(self):
        ...
        try:
            self.target_freq, self.waveform = FrequencyManager.validate_frequency(self.target_freq, self.monitor_refresh_rate, self.waveform)
    ...
    def analyze_session(self):
        ...
        try:
            with open(filename, 'w', newline='') as csvfile:
                ...
                # Header
                writer.writerow(["Frame_ID", "Timestamp_Monotonic", "Delta_Time_Sec", "Luminance_NDC", "Target_Freq_Hz", "Sham_Mode"])
                # Data
